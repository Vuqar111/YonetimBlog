...
import { productDelete, productList } from '../redux/slices/productSlice';
interface Product {
  id: number;
  name: string;
  subtitle: string;
  ...
  const handleDelete = async (id: number) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        await dispatch(productList({ searchQuery }));
        ...
        {data?.map((product, i) => {
          return (
            ...
            >
              <div className="col-span-2 flex items-center">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                  <div className="h-12.5 w-15 rounded-md">
                    {product?.image ? (
                      <img className='max-h-[40px] max-w-[40px]' src={product?.image} alt="Product" />
                    ) : (
                      <img className='max-h-[40px] max-w-[40px]' src={`https://kabelmarket-admin.vercel.app${product?.image}`} alt="Product" />
                    )}
                  </div>
              ...
              <div className="col-span-2 flex items-center">
                <p className="text-sm text-meta-3">{product?.subtitle}</p>
              </div>
              
              <div className="col-span-2 flex items-center space-x-3.5">
                <Link
                  to={'/blog-edit/' + product.id}
